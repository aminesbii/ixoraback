...
export const getClicks = async (req, res) => {
  try {
    const product = await productService.getProductClicks(req.params.id);
    if (!product) return res.status(404).json({ message: "Product not found." });
    res.json({ product_id: product.id, name: product.name, clicks: product.clicks });
  } catch (err) {
    console.error("[Product] GetClicks error:", err);
    res.status(500).json({ message: "Internal server error." });
  }
};
