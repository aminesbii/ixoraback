...
const READ_WINDOW_MS = Number(
  process.env.RATE_READ_WINDOW_MS ?? (isProd ? 60_000 : 10_000)
);
const READ_LIMIT = Number(process.env.RATE_READ_LIMIT ?? (isProd ? 2000 : 500));
const WRITE_WINDOW_MS = Number(
  process.env.RATE_WRITE_WINDOW_MS ?? (isProd ? 15 * 60_000 : 60_000)
);
const WRITE_LIMIT = Number(process.env.RATE_WRITE_LIMIT ?? (isProd ? 200 : 80));
const AUTH_WINDOW_MS = Number(
  process.env.RATE_AUTH_WINDOW_MS ?? (isProd ? 15 * 60_000 : 60_000)
);
...
const limitMethods = (limiter, methods) => (req, res, next) =>
